...
export const AssignmentPage: React.FC<AssignmentPageProps> = ({
  ...
  const filteredItems = receipt.items.filter((item) => {
    const count = assignments[item.id]?.assigned_members.length || 0;
    if (filter === 'unassigned') return count === 0;
    if (filter === 'assigned') return count > 0;
    return true;
  });
  ...
  const handleSubmit = () => {
    if (!allAssigned) {
      alert('Please assign all items to at least one diner before proceeding.');
      return;
    }
    onCalculateSplit(Object.values(assignments));
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Tap diners who shared each dish. Split equally by default or set custom % proportions.
          </p>
        </div>

        {/* Unassigned Counter Pill */}
        <div
          className={`px-4 py-2.5 rounded-2xl text-sm font-bold border flex items-center gap-2 shadow-sm ${
            allAssigned
      ...
      {/* Filter Tabs */}
      <div className="flex items-center gap-2 mb-6 bg-slate-900/60 p-1.5 rounded-2xl border border-slate-800 w-fit">
        <button
        ...
        >
          Unassigned ({receipt.items.length - assignedCount})
        </button>
      ...
      <div className="space-y-4 mb-8">
        {filteredItems.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`glass-panel rounded-3xl p-6 border transition-all duration-200 shadow-xl ${
                isItemAssigned ? 'border-slate-800 bg-slate-900/60' : 'border-amber-500/40 bg-amber-950/10'
              }`}
            >
              {/* Item Top Row */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-3 mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-slate-800/90 hover:bg-slate-700 text-xs font-bold text-indigo-400 border border-indigo-500/30 transition-colors flex items-center gap-1.5 shadow-sm"
                  >
              ...
              {/* Diners Selection Buttons */}
              <div className="space-y-3">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
                  Select Diners Who Ordered This:
                </span>
                  ...
                  {members.map((m) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-2.5 rounded-2xl text-sm font-bold flex items-center gap-2 transition-all duration-200 border ${
                          isSelected
                      ...
                      >
                        ...
                        {m.name}
                        {isSelected && <Check className="w-4 h-4 text-indigo-200 stroke-[2.5]" />}
                      </button>
                    );
                  })}
                </div>
                    ...
                    {assignment.split_type === 'custom' && (
                      <div className="flex items-center gap-2 flex-wrap text-xs">
                        {assignment.assigned_members.map((mId) => {
                          ...
                          return (
                            <div key={mId} className="flex items-center gap-1 bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-700">
                              ...
                              <input
                                ...
                                className="w-12 bg-slate-950 text-indigo-300 font-extrabold px-1 py-0.5 rounded text-right focus:outline-none"
                              />
      ...
      <div className="sticky bottom-6 glass-panel rounded-3xl p-5 border border-slate-800 shadow-2xl flex items-center justify-between gap-4">
        <div>
          <span className="text-base font-extrabold text-slate-100 block">Ready to Calculate?</span>
        ...
        <button
          ...
          className="btn-primary px-7 py-3.5 flex items-center gap-2 text-base font-bold shadow-xl"
        >
